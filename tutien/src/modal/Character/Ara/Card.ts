...
export interface AraCardConfig {
...
export function createAraCard(config: AraCardConfig): {
  el: HTMLDivElement
  displayEl: HTMLDivElement
} {
  const el = document.createElement('div')
  el.className = 'ara-card'
  ...
  name.textContent = config.name ?? 'Ara'
  ...
  if (config.onAttack) {
    attackBtn.addEventListener('click', config.onAttack)
  }
  ...
  if (config.onAttack2) {
    attackBtn2.addEventListener('click', config.onAttack2)
  }
  ...
  if (config.onSpecSkill) {
    specSkillBtn.addEventListener('click', config.onSpecSkill)
  }
