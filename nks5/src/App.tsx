  ...
  const fetchSubmissions = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const itemsWithId: SubmissionItem[] = data.data.map((item: any, idx: number) => ({
          ...item,
          id: total - idx,
        }));
        setSubmissions(itemsWithId);
        setCount(data.count);
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#0b0f19] text-slate-100 flex flex-col justify-start selection:bg-amber-500 selection:text-white relative overflow-x-hidden">
      ...
      <header className="w-full border-b border-slate-800/80 bg-slate-900/90 backdrop-blur-xl sticky top-0 z-50 shadow-2xl">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2.5 flex justify-between items-center gap-2">
          ...
          {/* Perfectly Aligned Icon Buttons */}
          <div className="flex items-center space-x-2 shrink-0">
            ...
            <a
              ...
              download
              title="Tải file .txt"
              className="w-8 h-8 sm:w-9 sm:h-9 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white rounded-xl shadow-md shadow-emerald-500/20 transition-all flex items-center justify-center cursor-pointer border border-emerald-400/30"
            >
              <svg className="w-4 h-4 sm:w-4.5 sm:h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <tr key={item.id} className="hover:bg-amber-500/10 transition-colors">
                        ...
                        <td className="px-2 sm:px-3 py-2 font-bold text-sky-300 whitespace-nowrap">
                          <div className="flex items-center space-x-1.5">
                            ...
                            <span className="truncate max-w-[120px] sm:max-w-none">{item.zalo || item.raw}</span>
                          </div>
                        </td>
